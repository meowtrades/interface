...
const ColorMap: Record<string, { bg: string, text: string, primary: string, gradient: string }> = {
  'dca': { bg: 'bg-blue-100', text: 'text-blue-600', primary: '#3b82f6', gradient: '#93c5fd' },
  'grid': { bg: 'bg-purple-100', text: 'text-purple-600', primary: '#8b5cf6', gradient: '#c4b5fd' },
  'momentum': { bg: 'bg-amber-100', text: 'text-amber-600', primary: '#d97706', gradient: '#fcd34d' },
  'custom': { bg: 'bg-green-100', text: 'text-green-600', primary: '#22c55e', gradient: '#86efac' },
};
...
const StrategySimulationDialog = ({ 
  ...
  const colorScheme = ColorMap[strategy.type] || { bg: 'bg-gray-100', text: 'text-gray-600', primary: '#4ade80', gradient: '#86efac' };
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-slate-50 p-4 rounded-lg text-center">
              <p className="text-sm text-slate-500 mb-1 font-medium">Initial Investment</p>
              <p className="text-xl font-semibold">${initialInvestment.toFixed(2)}</p>
            </div>
            
            <div className="bg-slate-50 p-4 rounded-lg text-center">
              <p className="text-sm text-slate-500 mb-1 font-medium">Current Value</p>
            ...
            <div className={`p-4 rounded-lg text-center ${isProfitable ? 'bg-green-50' : 'bg-red-50'}`}>
              <p className="text-sm text-slate-500 mb-1 font-medium">Profit/Loss</p>
                ...
                <AreaChart data={performanceData} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                  <defs>
                    <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={colorScheme.primary} stopOpacity={0.2}/>
                  ...
                  <Area 
                    ...
                    fill="url(#colorValue)"
                    dot={false}
                    activeDot={{ r: 6, fill: colorScheme.primary, stroke: '#fff', strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="py-2 px-4 text-left font-medium text-slate-600">Type</th>
                    <th className="py-2 px-4 text-left font-medium text-slate-600">Amount</th>
                    <th className="py-2 px-4 text-left font-medium text-slate-600">Value</th>
                  </tr>
                </thead>
                <tbody>
                  {transactions.map((tx, i) => (
                    <tr key={i} className="border-t border-slate-200">
                      ...
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center px-3 py-1 ${colorScheme.bg} ${colorScheme.text} rounded-full text-sm`}>
                          {tx.type}
                        </span>
                      </td>
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200">
            ...
            <div className="space-y-3">
                ...
                <>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">The strategy purchased at regular intervals, reducing timing risk.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <DollarSign className={`${colorScheme.text} mt-0.5 w-5 h-5`} />
                    <div>
                ...
                <>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">The strategy buys at lower price levels and sells at higher price levels.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <ArrowRight className={`${colorScheme.text} mt-0.5 w-5 h-5`} />
                    <div>
              ...
              {strategy.type === 'momentum' && (
                <>
                  <div className="flex items-start gap-3">
                    <TrendingUp className={`${colorScheme.text} mt-0.5 w-5 h-5`} />
                    <div>
                      <p className="font-medium">Trend Following</p>
                      <p className="text-sm text-slate-500">The strategy buys when prices show upward momentum.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <RefreshCw className={`${colorScheme.text} mt-0.5 w-5 h-5`} />
                    <div>
                      <p className="font-medium">Dynamic Allocation</p>
                      <p className="text-sm text-slate-500">Position sizes are adjusted based on market conditions and trend strength.</p>
                    </div>
                  </div>
                </>
              )}
              
              {strategy.type !== 'dca' && strategy.type !== 'grid' && strategy.type !== 'momentum' && (
                <div className="flex items-start gap-3">
                  <RefreshCw className={`${colorScheme.text} mt-0.5 w-5 h-5`} />
                  <div>
                    <p className="font-medium">Custom Strategy</p>
                    <p className="text-sm text-slate-500">This strategy uses custom parameters to optimize your investment over time.</p>
                  </div>
                </div>
              )}
            </div>
