...
import { TrendingUp, TrendingDown, ArrowRight, BarChart2, Zap, RefreshCw, Grid } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const totalInvested = activeUserStrategies.reduce((total, us) => total + us.invested, 0);
  const totalValue = activeUserStrategies.reduce((total, us) => total + us.currentValue, 0);
  const totalProfit = totalValue - totalInvested;
  const profitPercentage = totalInvested > 0 ? ((totalProfit / totalInvested) * 100) : 0;
  const isProfitable = totalProfit > 0;
  
  // Count mock trades
  const mockTradesCount = 3; // In a real app, this would come from the API
    ...
    <AppLayout>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            {isLoading ? (
              ...
              <CardTitle className="text-2xl">{activeUserStrategies.length}</CardTitle>
            )}
          </CardHeader>
          <CardContent className="pb-2">
            <div className="text-sm text-slate-500">{mockTradesCount} mock trades</div>
          </CardContent>
        </Card>
      </div>
          ...
          {isLoading ? (
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`px-3 py-1 rounded-full text-white text-xs font-medium ${
                        strategy.type === 'dca' ? 'bg-blue-600' : 
                        strategy.type === 'grid' ? 'bg-purple-600' : 
                        'bg-amber-600'
                      }`}>
                        {strategy.type === 'dca' ? 'DCA' : 
                         strategy.type === 'grid' ? 'Grid' : 
                         'Momentum'}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">${userStrategy.currentValue.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 text-sm">Starting Value</span>
                        <span className="font-medium">${userStrategy.invested.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 text-sm">Profit/Loss</span>
                        <span className={`font-medium ${userStrategy.profit >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                          {userStrategy.profit >= 0 ? '+' : '-'}${Math.abs(userStrategy.profit).toFixed(2)} ({userStrategy.profitPercentage.toFixed(2)}%)
                        </span>
                      </div>
                    </div>
                  </CardContent>
                  <CardFooter>
                    <Link to={`/app/strategies/${userStrategy.strategyId}`} state={{ source: 'dashboard' }} className="w-full">
            ...
            <Card className="border-dashed border-2 border-slate-200">
              <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
                ...
                <h3 className="text-lg font-medium mb-2">Start Your First Strategy</h3>
                <p className="text-slate-500 mb-4">
                  Choose from our range of automated trading strategies to start investing with just one click.
                </p>
          ...
          {activeUserStrategies.length > 0 && (
            <Card className="border-dashed border-2 border-slate-200">
              <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
                ...
                <h3 className="text-lg font-medium mb-2">Start a New Strategy</h3>
                <p className="text-slate-500 mb-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/app/history" className="text-blue-600 text-sm flex items-center">
            View all <ArrowRight size={16} className="ml-1" />
          </Link>
        </div>
                ...
                {[
                  ...
                  <div key={activity.id} className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">{activity.type} {activity.token}</div>
